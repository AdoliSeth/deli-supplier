...
//  create query
$query = 'SELECT * FROM supproductsa ORDER BY created_at DESC';
...
//fetch data
if (!empty($result)) {
    $supproductsa = mysqli_fetch_all($result, MYSQLI_ASSOC);
}
...
<section>
    <div class="container">
        ...
        <div class="row">
            <?php foreach($supproductsa as $post):  ?>
...
<!--                            <input type="hidden" name="hidden_price" class="--><?php //echo $posts['price']?><!--">-->
                            ...
                            <a class="btn btn-default" style="background-color: darkorange" href="<?php echo ROOT_URL;?>supacartthree.php?id=<?php echo $post['id'];?>">Add to Cart</a>
                        </div>
                    </div>
